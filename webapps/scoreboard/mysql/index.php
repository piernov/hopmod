...
function show_player($db, $player, $order, $admin) {
	...
	while ($row = mysql_fetch_array($results)) {
		...
		echo '<tr>';
		echo '<td>';
		echo '<a href="game.php?id='.$row["game_id"].'">'.$row["datetime"].'</a>';
		echo '</td>';
		echo '<td>';
		echo $row["gamemode"];
		echo '</td>';
		...
		echo '</td>';
		echo '<td>';
		echo formatAccuracy($row["hits"], $row["shots"]);
		echo '</td>';
		echo '<td>';
		echo $row["suicides"];
		echo '</td>';
...
function show_players($db, $order, $country = NULL, $player = NULL, $norank = FALSE, $admin) {
	...
	$results = mysql_query('SELECT * FROM playertotals ORDER BY '.$order.' DESC');
	echo '
